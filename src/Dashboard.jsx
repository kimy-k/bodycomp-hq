...
/* ═══ SHARED UI ═══ */
const Tip=({active,payload,label})=>!active||!payload?.length?null:(<div style={{background:"rgba(15,15,26,0.95)",backdropFilter:"blur(12px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"12px 16px",fontSize:12,color:"#e0e0e0",fontFamily:"Outfit",boxShadow:"0 8px 32px rgba(0,0,0,0.4)"}}><div style={{fontWeight:700,marginBottom:6,color:"#fff",fontSize:13}}>{label}</div>{payload.map((p,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:8,marginTop:4}}><span style={{width:8,height:8,borderRadius:"50%",background:p.color,display:"inline-block",boxShadow:`0 0 8px ${p.color}66`}}/><span style={{opacity:0.5}}>{p.name}</span><span style={{fontWeight:700,color:p.color,marginLeft:"auto"}}>{p.value}</span></div>))}</div>);
const Card=({title,value,unit,sub,color,icon})=>(<div className="fade-in" style={{background:"linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:"16px 18px",flex:"1 1 140px",minWidth:140,backdropFilter:"blur(8px)"}}><div style={{fontSize:10,color:"rgba(255,255,255,0.35)",textTransform:"uppercase",letterSpacing:1.5,marginBottom:6,fontWeight:600,fontFamily:"Outfit"}}>{icon} {title}</div><div style={{fontSize:28,fontWeight:800,color,letterSpacing:"-1px",fontFamily:"Outfit"}}>{value}<span style={{fontSize:13,fontWeight:400,opacity:0.5,marginLeft:2}}>{unit}</span></div>{sub&&<div style={{fontSize:10,marginTop:4,color:"rgba(255,255,255,0.25)",fontFamily:"Outfit"}}>{sub}</div>}</div>);
const H2=({children,sub})=>(<div style={{marginBottom:14,marginTop:32}}><h2 style={{fontSize:18,fontWeight:800,color:"#fff",margin:0,letterSpacing:"-0.3px",fontFamily:"Outfit"}}>{children}</h2>{sub&&<p style={{fontSize:11,color:"rgba(255,255,255,0.25)",margin:"3px 0 0",fontFamily:"Outfit"}}>{sub}</p>}</div>);
const TabBtn=({active,onClick,children})=>(<button onClick={onClick} style={{padding:"7px 12px",borderRadius:20,border:"none",cursor:"pointer",fontSize:11,fontWeight:active?700:500,fontFamily:"Outfit",background:active?"rgba(167,139,250,0.15)":"transparent",color:active?"#a78bfa":"rgba(255,255,255,0.3)",outline:active?"1px solid rgba(167,139,250,0.2)":"none",whiteSpace:"nowrap",transition:"all 0.2s"}}>{children}</button>);
const Ring=({pct,color,size=64,stroke=6,children})=>{const r=(size-stroke)/2;const ci=2*Math.PI*r;return(<div style={{position:"relative",width:size,height:size}}><svg width={size} height={size} style={{transform:"rotate(-90deg)"}}><circle cx={size/2} cy={size/2} r={r} fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth={stroke}/><circle cx={size/2} cy={size/2} r={r} fill="none" stroke={pct>100?"#f87171":color} strokeWidth={stroke} strokeDasharray={`${ci*Math.min(pct,100)/100} ${ci}`} strokeLinecap="round" style={{filter:`drop-shadow(0 0 4px ${color}44)`,transition:"stroke-dasharray 0.8s ease"}}/></svg><div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>{children}</div></div>);};
const Insight=({icon,title,text,color})=>(<div className="fade-in" style={{background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",border:"1px solid rgba(255,255,255,0.05)",borderRadius:16,padding:"14px 16px",marginBottom:10,backdropFilter:"blur(4px)"}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{fontSize:16}}>{icon}</span><span style={{fontSize:14,fontWeight:700,color,fontFamily:"Outfit"}}>{title}</span></div><div style={{fontSize:12,lineHeight:1.7,color:"rgba(255,255,255,0.4)",fontFamily:"Outfit"}}>{text}</div></div>);
const cBox={background:"linear-gradient(135deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))",borderRadius:18,padding:"16px 8px 8px",border:"1px solid rgba(255,255,255,0.04)",backdropFilter:"blur(4px)"};
const inp={width:"100%",padding:"10px 12px",borderRadius:12,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:14,fontFamily:"Outfit",outline:"none",boxSizing:"border-box",transition:"border-color 0.2s"};
